	...
	<view class="uni-container">
		<uni-forms ref="form" :model="formData" validateTrigger="bind">
			...
			<uni-forms-item name="open_url" label="目标地址" :label-width="labelWidth" label-align="right">
				<uni-easyinput placeholder="点击跳转目标地址。如果是web地址则使用内置web-view打开；如果是本地页面则跳转本地页面；如果是schema地址则打开本地的app"
					v-model="formData.open_url" trim="both"></uni-easyinput>
			</uni-forms-item>
			...
			<uni-forms-item name="description" label="备注" :label-width="labelWidth" label-align="right">
				<uni-easyinput type="textarea" placeholder="请填写备注" v-model="formData.description" trim="both"
	...
	export default {
		...
		methods: {
			...
			getDetail(id) {
				...
				db.collection(dbCollectionName).doc(id).field("image,open_url,title,sort,status,description").get().then((
				res) => {
