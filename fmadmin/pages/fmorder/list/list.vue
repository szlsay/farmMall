	...
	<view>
		...
		<view class="uni-container">
			<unicloud-db ref="udb" :collection="collectionList"
				field="oid,create_time,price_amount_total,state,order_delivery,order_goodslist" :where="where"
				...
				<uni-table ref="table" :loading="loading" :emptyText="error.message || '没有更多数据'" border type="selection"
					...
					<uni-tr>
						...
						<uni-th align="center" filter-type="range" @filter-change="filterChange($event, 'price_amount_total')"
							sortable @sort-change="sortChange($event, 'price_amount_total')">总合计</uni-th>
						<uni-th align="center" filter-type="select" :filter-data="options.filterData.state_localdata"
							@filter-change="filterChange($event, 'state')">订单类型</uni-th>
						<uni-th align="center" sortable @sort-change="sortChange($event, 'order_delivery')">收货信息</uni-th>
	...
	export default {
		data() {
			return {
				...
				exportExcel: {
					"filename": "fm-order.xls",
					"type": "xls",
					"fields": {
						"订单id": "oid",
						"创建时间": "create_time",
						"总合计": "price_amount_total",
						"订单类型": "state",
						"order_delivery": "order_delivery",
						"商品列表": "order_goodslist"
					}
				},
				exportExcelData: []
			}
